...
object Demo4AddingDatabase extends App {
  ...
  lazy val AuthorType = deriveObjectType[Unit, Author]()

  // NEW: use repository type a context object
  val QueryType = ObjectType("Query", fields[BookRepo with AuthorRepo, Unit](
    Field("books", ListType(BookType), description = Some("Gives the list of books sorted and filtered based on the arguments"),
